...
public class ProcessingController {
     ...
     * 
     * This endpoint simulates the complete architecture:
     * 1. Controller receives request (Layer 1 - On-premises monolith)
     * 2. Calls Lambda service (Layer 2 - AWS Lambda Python microservice) 
     * 3. Calls EKS service (Layer 3 - EKS Java Spring microservice)
     * 4. Performs database operation (Data layer)
     * 5. Executes complex business logic (Business layer)
     * 
     * All with comprehensive tracing, resilience patterns, and business context.
     */
    ...
    public ResponseEntity<ProcessingResult> completeProcessingFlow(
        ...
        logger.info("Starting complete processing flow for user: {} with request: {} (correlationId: {})", 
                   userId, request.getData(), correlationId);
        ...
        logger.info("Processing with business context - TransactionId: {}, CorrelationId: {}, ProductCode: {}",
            businessTransactionId, businessCorrelationId, currentBaggage.getEntryValue("business.product.code"));
        ...
        try {
            ...
            ProcessingResult lambdaResult = externalServiceClient.callLambdaService(userId, request.getData());
            logger.info("Lambda service completed: {} (resultId: {})", lambdaResult.getMessage(), lambdaResult.getResultId());
            
            // Step 2: Call EKS service (Bulkhead, Circuit Breaker, Retry)
            logger.debug("Step 2: Calling EKS service");
            ProcessingResult eksResult = externalServiceClient.callEksService(
                businessTransactionId, 
                lambdaResult.getResultId()
            );
            logger.info("EKS service completed: {} (resultId: {})", eksResult.getMessage(), eksResult.getResultId());
            
            // Step 3: Async database operation (Time Limiter, Circuit Breaker, Retry)
            logger.debug("Step 3: Performing database operation");
            CompletableFuture<ProcessingResult> dbOperation = externalServiceClient.performDatabaseOperationAsync(
                "SELECT * FROM processed_data WHERE transaction_id = '" + businessTransactionId + "'"
            );
            ProcessingResult dbResult = dbOperation.get(); // This respects TimeLimiter configuration
            logger.info("Database operation completed: {} (resultId: {})", dbResult.getMessage(), dbResult.getResultId());
            
            // Step 4: Complex business operation (multiple resilience patterns)
            logger.debug("Step 4: Executing complex business operation");
            ProcessingResult complexResult = externalServiceClient.complexBusinessOperation(
                request.getOperationId(), 
                request.getParameters()
            );
            logger.info("Complex operation completed: {} (resultId: {})", complexResult.getMessage(), complexResult.getResultId());
            ...
            ProcessingResult finalResult = ProcessingResult.builder()
                    ...
                    .message(String.format("Complete flow finished successfully - Lambda: %s, EKS: %s, DB: %s, Complex: %s",
                        lambdaResult.getResultId(), eksResult.getResultId(), 
                        dbResult.getResultId(), complexResult.getResultId()))
                    .status("SUCCESS")
            ...
            logger.info("Complete processing flow finished successfully for user: {} in {}ms - TxnId: {}", 
                       userId, totalDuration, businessTransactionId);
            
            return ResponseEntity.ok(finalResult);
            
        } catch (ExecutionException | InterruptedException e) {
            logger.error("Complete processing flow failed for user: {} - Error: {}", userId, e.getMessage(), e);
    ...
    /**
     * Endpoint demonstrating caching behavior with Rate Limiter.
     * First call hits the service, subsequent calls use cache.
     */
    ...
    public ResponseEntity<ProcessingResult> getUserData(
        ...
        try {
            ...
            logger.info("User data fetch completed for userId: {} (cached: {})", 
                       userId, result.getMetadata().getOrDefault("cache_hit", false));
    ...
    /**
     * Endpoint demonstrating heavy processing with Thread Pool Bulkhead isolation.
     * Uses separate thread pool to prevent resource exhaustion.
     */
    ...
    public ResponseEntity<ProcessingResult> performHeavyProcessing(
        ...
        logger.info("Starting heavy processing for data: {} with size: {} MB (user: {})", dataId, dataSizeMb, userId);
        
        try {
            // This uses Thread Pool Bulkhead to isolate heavy operations
            CompletableFuture<ProcessingResult> heavyResult = externalServiceClient.performHeavyProcessing(dataId, dataSizeMb);
            ...
            logger.info("Heavy processing completed for data: {} in {}ms", dataId, result.getProcessingTimeMs());
    ...
    /**
     * Endpoint to test individual resilience patterns.
     * Allows testing specific patterns in isolation.
     */
        ...
        try {
            ...
            switch (pattern.toLowerCase()) {
                ...
                case "retry":
                    ...
                    result = externalServiceClient.callEksService("test-txn-" + System.currentTimeMillis(), data);
                    break;
                ...
                case "bulkhead":
                    ...
                    result = externalServiceClient.callEksService("bulkhead-test-" + System.currentTimeMillis(), data);
                    break;
                    
                case "time-limiter":
                case "tl":
                    // Test database time limiter
                    CompletableFuture<ProcessingResult> dbResult = externalServiceClient.performDatabaseOperationAsync(
    ...
    public ResponseEntity<ProcessingResult> healthCheck() {
        ...
        ProcessingResult healthResult = ProcessingResult.builder()
                .resultId("health-ok-" + System.currentTimeMillis())
                .message("Service is healthy and all resilience patterns are active")
                .status("SUCCESS")
                .sourceService("distributed-tracing-poc")
                .operationType("HEALTH_CHECK")
                .addBusinessContext("product_code", currentBaggage.getEntryValue("business.product.code"))
                ...
                .addMetadata("resilience_patterns", "circuit-breaker,retry,rate-limiter,bulkhead,time-limiter,cache")
    ...
    public ResponseEntity<ProcessingResult> getSystemInfo() {
        ...
        ProcessingResult infoResult = ProcessingResult.builder()
                ...
                .addMetadata("spring_boot_version", org.springframework.boot.SpringBootVersion.getVersion())
