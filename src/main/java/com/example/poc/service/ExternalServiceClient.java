...
public class ExternalServiceClient {
     ...
     * 
     * Circuit Breaker: Opens when failure rate exceeds 60% in the last 10 calls
     * Retry: Retries up to 4 times with exponential backoff starting at 500ms
     * Rate Limiter: Allows max 15 calls per second with 100ms wait time
     * 
     * This represents Layer 2 in the distributed architecture (Lambda microservice).
     */
    ...
    /**
     * Fallback method for Lambda service failures.
     * Provides graceful degradation when Lambda service is unavailable.
     */
     ...
     * 
     * Bulkhead: Limits concurrent executions to 3 (semaphore-based isolation)
     * Circuit Breaker: Opens when failure rate exceeds 40% in the last 8 calls
     * Retry: Retries up to 3 times with 2s initial delay and exponential backoff
     * 
     * This represents Layer 3 in the distributed architecture (EKS microservice).
     */
    ...
    public ProcessingResult callEksService(String transactionId, String payload) {
        logger.info("Calling EKS service for transaction: {} with payload size: {} bytes", 
                   transactionId, payload != null ? payload.length() : 0);
    ...
    public ProcessingResult fallbackEksCall(String transactionId, String payload, Exception ex) {
        logger.warn("EKS service fallback triggered for transaction: {} - Reason: {}", transactionId, ex.getMessage());
     ...
     * 
     * Time Limiter: Prevents operations from hanging beyond 5 seconds
     * Circuit Breaker: Opens when failure rate exceeds 30% in the last 12 calls
     * Retry: Retries up to 2 times with 3s initial delay
     * 
     * Uses CompletableFuture for async operations with time limits.
     */
    ...
    public CompletableFuture<ProcessingResult> fallbackDatabaseCall(String query, Exception ex) {
        logger.warn("Database operation fallback triggered for query: {} - Reason: {}", 
                   maskSensitiveQuery(query), ex.getMessage());
     ...
     * 
     * Cache: Results cached for 5 minutes (300s) with max 1000 entries
     * Rate Limiter: Allows max 20 calls per second with 500ms wait time
     * 
     * Cache key is automatically based on method parameters.
     */
    ...
    /**
     * Complex example combining multiple Resilience4j patterns:
     * Rate Limiter + Circuit Breaker + Thread Pool Bulkhead + Cache
     * 
     * This demonstrates how multiple patterns work together for comprehensive resilience.
     */
    ...
    public ProcessingResult complexBusinessOperation(String operationId, String parameters) {
        logger.info("Executing complex business operation: {} with params: {}", operationId, parameters);
     ...
     */
    public ProcessingResult fallbackCombinedCall(String operationId, String parameters, Exception ex) {
        logger.warn("Complex operation fallback triggered for operation: {} - Reason: {}", operationId, ex.getMessage());
    ...
    /**
     * Heavy processing service demonstrating Thread Pool Bulkhead isolation.
